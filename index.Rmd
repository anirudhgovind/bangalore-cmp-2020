...
junctionImprovement <- junctionImprovement %>% 
  select(Type, Description, `Budget(in Cr.)`, ID, geometry)
...
offStreetParking <- offStreetParking %>% 
  select(Type, Description, ID, Area, Count, `Budget(in Cr.)`, geometry)
...
mapOffStreetParking <- offStreetParking %>% 
  ...
  tm_dots(col = "#277da1",
          ...
          alpha = 0.5,
          border.alpha = 0,
          popup.vars = c("ID", "Type", "Description", "Area", "Count", "Budget(in Cr.)"))
...
stationAreaAccess <- stationAreaAccess %>% 
  select(Type, Description, ID, `Budget(in Cr.)`, geometry)
...
interModalHubs <- interModalHubs %>% 
  select(Type, Description, ID, Interchange, `Budget(in Cr.)`, geometry)
...
mapInterModalHubs <- interModalHubs %>% 
  tm_shape() +
  tm_dots(col = "#90be6d",
          popup.vars = c("ID", "Type", "Description", "Interchange", "Budget(in Cr.)"))
...
# Filter only the foot-over-bridge projects

footOverBridges <- geoRefProjects %>% 
  filter(type == "Foot-Over-Bridges")
...
# Draw foot-over-bridges

mapFootOverBridges <- footOverBridges %>% 
  tm_shape() +
  tm_dots(col = "#f9c74f",
          size = 0.15,
          alpha = 0.5,
...
mapBase + mapJunctionImprovement + mapOffStreetParking + mapStationAreaAccess + mapInterModalHubs + mapFootOverBridges
